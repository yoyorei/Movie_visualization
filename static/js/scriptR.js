function createBubbleChart(error, countries, continentNames) {
  var scores = countries.map(function(country1) { return +country1.num_voted_users; });
  var meanScore = d3.mean(scores),
      scoreExtent = d3.extent(scores),
      scoreScaleX,
      scoreScaleY;

  var continents = d3.set(countries.map(function(country1) { return country1.ContinentCode; }));
  ...
  function createCircles() {
    var formatScore = d3.format(",");
    circles = svg.selectAll("circle")
      .data(countries)
      .enter()
        .append("circle")
        .attr("r", function(d) { return circleRadiusScale(d.num_voted_users); })
    ...
    function updateCountryInfo(country1) {
      var info = "";
      if (country1) {
        info = [country1.country, formatScore(country1.num_voted_users)].join(": ");
      }
  ...
  function createForces() {
    ...
    function createScoreForces() {
      var continentNamesDomain = continents.values().map(function(continentCode) {
        return continentNames[continentCode];
      });
      var scaledScoreMargin = circleSize.max;

      scoreScaleX = d3.scaleBand()
        .domain(continentNamesDomain)
        .range([scaledScoreMargin, width - scaledScoreMargin*2]);
      scoreScaleY = d3.scaleLog()
        .domain(scoreExtent)
        .range([height - scaledScoreMargin, scaledScoreMargin*2]);

      var centerCirclesInScaleBandOffset = scoreScaleX.bandwidth() / 2;
      return {
        x: d3.forceX(function(d) {
            return scoreScaleX(continentNames[d.ContinentCode]) + centerCirclesInScaleBandOffset;
          }).strength(forceStrength),
        y: d3.forceY(function(d) {
          return scoreScaleY(d.num_voted_users);
        }).strength(forceStrength)
      };
    }
  ...
  function forceCollide(d) {
    return countryCenterGrouping() || scoreGrouping() ? 0 : circleRadiusScale(d.num_voted_users) + 1;
  }
  ...
  function addFillListener() {
    d3.selectAll('input[name="fill"]')
      .on("change", function() {
        toggleContinentKey(!flagFill() && !scoreGrouping());
        updateCircles();
      });
  }
  ...
  function addGroupingListeners() {
    ...
    addListener("#score",      forces.score);

    function addListener(selector, forces) {
      d3.select(selector).on("click", function() {
        updateForces(forces);
        toggleContinentKey(!flagFill() && !scoreGrouping());
        toggleScoreAxes(scoreGrouping());
      });
    }
    ...
    function toggleScoreAxes(showAxes) {
      ...
      function createAxes() {
        ...
        var xAxis = d3.axisBottom(scoreScaleX)
        ...
        var yAxis = d3.axisLeft(scoreScaleY)
          .ticks(numberOfTicks, tickFormat);
        svg.append("g")
          .attr("class", "y-axis")
